...
const Admin = () => {
  ...
  const editProduct=async(productId)=>{
    ...
    try {
      ...
      let {data}= await axios.patch("http://localhost:8179/admin/edit-service",body,{    withCredentials: true,
      headers: {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'
  }})
  if(data.status){
    toast({
  ...
  const deleteProduct=async(delId)=>{
    try{
     let {data}= await axios.delete(`http://localhost:8179/admin/delete-service/${delId}`, {
        withCredentials: true,
        headers: {'Access-Control-Allow-Origin': '*', 'Content-Type': 'application/json'
    }})
    if(data.status){
  ...
  return (
    ...
    {
      data?.map((ele)=>(
        <AdminProduct data={ele} newText={newText} setNewText={setNewText} editProduct={editProduct} deleteProduct={deleteProduct}/>
      ))
    }
    </Box>
